...
from datasets.modelnet40_train import ModelNet40_Train
...
def get_arguments():
    ...
    parser.add_argument("--n_cluster", type=int, default=5, help="number of local clustered parts for a 3D object")
    ...
    parser.add_argument("--pretrained", default='', type=str, help="open clip version",)
    ...
    parser.add_argument("--slip-ckpt-path", type=str, default="")
    ...
    parser.add_argument('--objaverse_lvis_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--omniobject3d_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--scanobjnn_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--scanobjectnn_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--sonn_c_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--sonn_variant', type=str, default='hardest', help='')
    parser.add_argument('--modelnet40_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--modelnet_c_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--modelnet40_c_root', type=str, default='path/to/your_data', help='')
    parser.add_argument('--snv2_c_root', type=str, default='path/to/your_data', help='')
...
@torch.no_grad()
def clip_features_extraction(args, classnames, template, clip_model):
    all_embeddings = []
...
def clip_classifier(args, classnames, template, clip_model):
    clip_weights = []
    #classnames: bsz x [class1,class2,...class40]
    for classname in classnames:
...
@torch.no_grad()
def get_uni3d_logits(cache_type, feat, lm3d_model, clip_weights, p_thres=0.1,conf=False):
...
@torch.no_grad()
def get_ulip_logits(cache_type, feat, lm3d_model, clip_weights,clip_features=None,p_thres=0.1,conf=False):
    if isinstance(feat, list):
        feat = torch.cat(feat, dim=0).cuda()
    else:
        feat = feat.cuda()
...
def get_logits(args, feat, lm3d_model, clip_weights,clip_features=None,conf=False):
    if args.lm3d == 'uni3d':
        ...
        return get_ulip_logits(args.cache_type, feat, lm3d_model, clip_weights, p_thres=args.p_thres,clip_features=clip_features,conf=conf)
    else:
...
def build_test_data_loader(args, dataset_name, root_path, preprocess_val):
    ...
    if dataset_name == 'modelnet40':
    ...
    elif dataset_name == 'modelnet40_train':
        dataset = ModelNet40_Train(args,0)
        test_loader = DataLoader(dataset,batch_size=1,num_workers=2,shuffle=True)
        ...
        test_loader = DataLoader(dataset, batch_size=1, num_workers=10, shuffle=True)
...
class PairedDataset(torch.utils.data.Dataset):
    def __init__(self, clean_dataset, noisy_dataset):
        assert len(clean_dataset) == len(noisy_dataset)
    ...
    def __getitem__(self, idx):
        pc_clean, target, _, rgb_clean = self.clean_dataset[idx]
        pc_noisy, target2, _, rgb_noisy = self.noisy_dataset[idx]
        return (pc_clean, rgb_clean), (pc_noisy, rgb_noisy), target

def get_original_dataloader(args,intensity=0):
    if args.cor_type != 'clean':
        if "modelnet" in args.dataset:
            clean_dataset = ModelNet40_Train(args,intensity=0)
            noisy_dataset = ModelNet40_Train(args,intensity=1,noise_type='jitter')
            if args.lm3d == 'openshape':
                batch_size = 128
            else:
                batch_size = 64
        else:
            clean_dataset = ScanObjectNN_Train(args,intensity=0)
            noisy_dataset = ScanObjectNN_Train(args,intensity=1,noise_type='jitter')
            if args.lm3d == 'openshape':
                batch_size = 128
            else:
                batch_size = 64 #128 for openshape
    else:
        if "modelnet" in args.dataset:
            clean_dataset = ModelNet40_Train(args,intensity=0)
            noisy_dataset = ModelNet40_Train(args,intensity=0.2,noise_type='drop_global')
            if args.lm3d == 'openshape':
                batch_size = 128
            else:
                batch_size = 64
            ...
            clean_dataset = ScanObjectNN_Train(args,intensity=0)
            noisy_dataset = ScanObjectNN_Train(args,intensity=0.2,noise_type='drop_global')
            if args.lm3d == 'openshape':
                batch_size = 128
            else:
                batch_size = 64 #128 for openshape
    paired_dataset = PairedDataset(clean_dataset, noisy_dataset)
    loader = DataLoader(paired_dataset, batch_size=batch_size, num_workers=10, shuffle=False)
    return loader, clean_dataset.classnames, clean_dataset.template
...
def load_openshape(args):
    if args.oshape_version == 'vitg14':
        ...
        open_clip_model, _, _ = open_clip.create_model_and_transforms(clip_name,
                                                                      pretrained='./weights/CLIP-ViT-bigG-14-laion2B-39B-b160k/open_clip_pytorch_model.bin')
    ...
    checkpoint = torch.load(f"./weights/openshape-pointbert-vitg14-rgb/model.pt")
...
def load_ulip(args):
    ...
    ulip_ckpt_path = f'./3D_data/pointbert_{args.ulip_version}.pt'
